...
import (
	...
	"github.com/righstar2020/br-cti/pkg/constant"
...
func (svc *Service) GetAdminUserInfo(ctx context.Context, userId int) (*vo.AdminUserInfoVo, error) {
	...
	//格式化时间
	// 将时间戳转换为time.Time类型
    t := time.Unix(userInfoVo.LoginTime, 0)
    // 格式化时间
    formattedTime := t.Format("2006-01-02 15:04:05")
	userInfoVo.LoginTimeFormat = formattedTime
...
func (svc *Service) UpdateAdminUser(ctx context.Context, params *dto.UpdateAdminUserRequest) error {
	...
	adminUser.Value = convert.Int(params.Value)
