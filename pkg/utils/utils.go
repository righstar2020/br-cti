...
import (
	...
	"github.com/righstar2020/br-cti/pkg/utils/gmd5"
...
func SaveFile(url string, dirname string) (string, error) {
	// 判断文件地址是否为空
	if gstr.Equal(url, "") {
		return "", errors.New("文件地址不能为空")
	}
	...
	// 判断是否本站文件
	if gstr.Contains(url, UploadUrl()) {
		// 是否临时文件
		if gstr.Contains(url, "temp") {
			// 创建目录
			dirPath := FilePath() + "/" + dirname + "/" + time.Now().Format("20060102")
			if !CreateDir(dirPath) {
				return "", errors.New("文件目录创建失败")
			}
			// 原始文件地址(绝对路径)
			oldPath := gstr.Replace(url, UploadUrl(), UploadPath())
			// 目标目录地址(绝对路径)
			newPath := FilePath() + "/" + dirname + gstr.Replace(url, UploadUrl()+"/temp", "")
			// 移动文件
			os.Rename(oldPath, newPath)
			return GetFileUrl(gstr.Replace(newPath, UploadPath(), "")), nil
		} else {
			// 非临时图片
			//path := gstr.Replace(url, ImageUrl(), "")
			return url, nil
